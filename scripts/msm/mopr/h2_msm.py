...
for p in range(len(n_pcca)):
    ...
    for r in range(len(randoms)):
        if to_do_random[r]:
            fimp = np.load(f'../../2_additional_s229_mopR/cog/saved_furf/fimp_hlabel2_{r}.npy')
        ...
        for n in range(len(n_clus)):
            ...
            if to_do_clus[r,n]:
                ...
                np.savez(f'saved_h2_msm/dtraj_{r}_{n_clus[n]}.npz', *dtrj)
    ...
    np.save(f'saved_h2_msm/pops_{n_pcca[p]}.npy', pops)
    np.save(f'saved_h2_msm/ids_{n_pcca[p]}.npy', ids)
    np.save(f'saved_h2_msm/kinetics_{n_pcca[p]}.npy', kinetics)
    np.save(f'saved_h2_msm/ginis_{n_pcca[p]}.npy', ginis)
