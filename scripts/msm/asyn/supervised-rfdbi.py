...
for i in range(len(randoms)):
    ...
    fspace = np.load(f'../../../2_different_system_idp_rg/saved_supervised/fimp_supervised_{i}.npy').argsort()[::-1][:200]
    fspace = traj_data[:,fspace]

    for cl in range(len(n_clus)):
        dtrj = np.load(f'saved_msm/dtraj_{i}_{n_clus[cl]}.npy')
            ...
            for pc in range(len(n_pcca)):
                ...
                try:
                    ...
                    if np.concatenate((mdis)).shape[0] == n_clus[cl]:
                        print(i,cl,ml,n_pcca[pc], ostates)
