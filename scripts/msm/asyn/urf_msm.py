...
for p in range(len(n_pcca)):
    ...
    for r in range(len(randoms)):
        if to_do_random[r]:
            tic2 = np.load(f'../../../2_different_system_idp_rg/saved_fidp/fimp_hlabel3_{r}.npy')
        ...
        for n in range(len(n_clus)):
            ...
            if to_do_clus[r,n]:
                ...
                np.save(f'saved_msm3/dtraj_{r}_{n_clus[n]}.npy', dtrj)
    ...
    np.save(f'saved_msm3/pops_{n_pcca[p]}.npy', pops)
    np.save(f'saved_msm3/ids_{n_pcca[p]}.npy', ids)
    np.save(f'saved_msm3/kinetics_{n_pcca[p]}.npy', kinetics)
    np.save(f'saved_msm3/ginis_{n_pcca[p]}.npy', ginis)
    np.save(f'saved_msm3/entropys_{n_pcca[p]}.npy', entropys)
    np.save(f'saved_msm3/cdiffs_{n_pcca[p]}.npy', cdiffs)
    np.save(f'saved_msm3/sdiffs_{n_pcca[p]}.npy', sdiffs)
