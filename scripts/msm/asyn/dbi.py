...
dbi = np.zeros(( len(n_clus), len(msm_lags), len(n_pcca) )) + np.nan

for cl in range(len(n_clus)):
    dtrj = np.load(f'saved_msm/dtraj_{n_clus[cl]}.npy')
        ...
        for pc in range(len(n_pcca)):
            ...
            try:
                ...
                if np.concatenate((mdis)).shape[0] == n_clus[cl]:
                    ...
                    dbi[cl,ml,pc] = mtr.davies_bouldin_score(traj_data, mdtrj)
