...
hc_hc = np.load('saved_hc/hc_average.npy')
...
for h in range(2,11):
    hlabels_ = extras.get_hc_dtraj(hc_hc, nids=h)

    for i in range(len(randoms)):
        _, xtest, _, ytest = splt(features, hlabels_, test_size=0.3, random_state=randoms[i])

        clf = pkl.load( open(f'saved_hc/clf_hc{h}_{i}.pkl','rb') )
...
np.save('saved_lc/lc1_hc.npy', lc1)
np.save('saved_lc/lc2_hc.npy', lc2)
