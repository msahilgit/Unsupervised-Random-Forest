...
create_nonsense = syn.synthetic_data(features, size=1)
create_nonsense.nonsense()
pfeatures, plabels = create_nonsense.get_output()

np.save('saved_synthetic_nonsense/pfeatures.npy', pfeatures)
np.save('saved_synthetic_nonsense/plabels.npy', plabels)
...
for i in range(len(randoms)):
    ...
    clfs.append(clf_)
    pkl.dump(clf_, open(f'saved_synthetic_nonsense/clf_nonsense{i}.pkl','wb'))

np.save('saved_synthetic_nonsense/accu_nonsense.npy', accu)


nonsense_pmt = pmt.calculate_condenced_pmt_multi( clfs, features, n_jobs=94 )
np.save('saved_synthetic_nonsense/nonsense_pmt_mean.npy', nonsense_pmt)


hc_nonsense = scc.hierarchy.linkage(nonsense_pmt, method='average')
np.save('saved_synthetic_nonsense/hc_nonsense.npy', hc_nonsense)
...
for h in range(2,11):
    hlabels_ = extras.get_hc_dtraj(hc_nonsense, nids=h)
    ...
    for i in range(len(randoms)):
        ...
        xtrain, xtest, ytrain, ytest = 0, 0, 0, 0
        pkl.dump(clf_, open(f'saved_synthetic_nonsense/clf_hlabel{h}_{i}.pkl','wb'))
        fimp = clf_.feature_importances_
        np.save(f'saved_synthetic_nonsense/fimp_hlabel{h}_{i}.npy', fimp)
        ...
        for k in range(len(dists)):
            ...
            hist_.append(hh[4])
            extents[k] = hh[:4]
        np.save(f'saved_synthetic_nonsense/hist_{h}_{i}.npy', hist_)
        np.save(f'saved_synthetic_nonsense/extents_{h}_{i}.npy', extents)



np.save('saved_synthetic_nonsense/accu_hc.npy', accu)
