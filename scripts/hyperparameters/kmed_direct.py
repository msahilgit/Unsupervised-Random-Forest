...
for h in range(2,11):
    ...
    _, klabels = cls.kmed_euclidean( features, nclus=h, init='random', max_iter=5000, n_jobs=94, random_seed=h )
    np.save(f'saved_kmed_direct/try_2/klabels_{h}.npy', klabels)
    ...
    for i in range(len(randoms)):
        ...
        pkl.dump(clf_, open(f'saved_kmed_direct/try_2/clf_klabel{h}_{i}.pkl','wb'))
        fimp = clf_.feature_importances_
        np.save(f'saved_kmed_direct/try_2/fimp_klabel{h}_{i}.npy', fimp)
        ...
        for k in range(len(dists)):
            ...
            hist_.append(hh[4])
            extents[k] = hh[:4]
        np.save(f'saved_kmed_direct/try_2/hist_{h}_{i}.npy', hist_)

        print(h,i)


np.save('saved_kmed_direct/try_2/accu_kmed.npy', accu)
