...
def get_dtraj_renumbered_outliers(dtraj, cutoff=None, display=True):
    ...
    '''
    oinds = get_dtraj_outliers(dtraj, cutoff, False, False)
...
def get_confusion_matrix(true, pred, nl):
    '''
    compute confusion matrix C(i,j): i->true and j->predicted.
    mainly usefull with multiclass labels, specially n>=3 or much larger and skewed data, where
    skl.metrics.confusion_matrix may fails, as some of the labels may not be part of pred (of true).
    *It assumes labelling is done in 0..n fashion.

    INPUTS::
        true - [arr] true labels 
        pred - [arr] predicted labels
        nl   - [int] number of label classes (0..n numbering)

    OUTPUTS::
        cfm  - [arr] confusion matrix

    '''
    if true.shape != pred.shape or true.max() >= nl or pred.max() >= nl:
    ...
    for i in range(nl):
        inds = np.where(true == i)[0]
        for j in range(nl):
            cfm[i,j] = np.where( pred[inds] == j )[0].shape[0]
