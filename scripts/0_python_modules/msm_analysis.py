...
def get_mdtrj(dtrj, mdis, output_type='concated'):
    ...
    if np.max(cdtrj) != np.max(np.concatenate((mdis))): raise ValueError()
...
def get_reweighted_pis(pis, weights):
    '''
    re-weighing of probabilities as per the weights to be used for
    gini impurity calculation of highly skewed data

    MATHS::
        wpis = (eq * pis / weights) / \Sum(eq * pis / weights)

    INPUTS::
        pis     - [arr] p*m array of probs
        weights - [arr] weights of shape m

    '''
    if pis.shape[1] != weights.shape[0] or np.min(weights) <= 0 or any([np.sum(weights) != np.sum(i) for i in pis]):
        raise ValueError('input incompatibility')

    eq = np.ones(weights.shape)/weights.shape[0]

    wpis = np.array([
        eq * np.divide(i, weights) for i in pis
        ])
    wpis = np.array([
        i/np.sum(i) for i in wpis
        ])
    return wpis
...
# 
#def get_dbi(fspace, mdtrj):
#    '''
#    calculates Davies Bouldin Index (DBI) of classes defined in mdtrj [concatenated metastable dtrj]
#    in units of euclidean distances in coordinates of fspace.
#
#    MATHS::
#        dbi = 1/k \Sum {k} max(j) \frac {r_i + r_j} {r_ij}
#
#        where:
#            k are states in mdtrj
#            r_i is intra_cluster distances
#            r_ij centroids distances between i and j states
#
#    INPUTS::
#        fspace - [2d-arr] feature space 
#        mdtrj  - [1d-arr] metastable trajectory, concatenated
#    OUTPUTS::
#        dbi    - [float] davies bouldin index
#
#    '''
#    if not fspace.shape[0] == mdtrj.shape[0]: raise ValueError('mismatch b/w fspace and mdtrj')
#
#    macros = np.unique(mdtrj)
#
#    centroids = np.zeros(( len(macros), fspace.shape[1] ))+np.nan
#    mean_intra = np.zeros(( len(macros) )) + np.nan
#    for a,k in enumerate(macros):
#        inds = np.where(mdtrj==k)[0]
#        centroids[a] = np.mean( fspace[inds], axis=0)
#        mean_intra[a] = np.mean( np.linalg.norm( fspace[inds] - centroids[a], axis=1) )
#
#    mean_intra = mean_intra[:,np.newaxis] + mean_intra
#    np.fill_diagonal(mean_intra, 0)
#    centroids = np.linalg.norm( centroids[:,np.newaxis] - centroids, axis=2)
#
#    mean_intra = np.divide(mean_intra, centroids)
#
#    return np.mean(np.max(mean_intra, axis=1))
#
#    
#
#
#
#
#def get_dist_cluster_diff(fspace, mdtrj):
#    '''
#    calculates the diff between intra_state distances and inter_state distances in units of euclidean
#    distances in coordinates of fspace as per states defined in mdtrj
#    MATHS::
#        coef = \frac { \Sum intra_state distance } { \Sum inter_state distances }
#    INPUTS::
#        fspace - [2d-arr] feature space 
#        mdtrj  - [1d-arr] metastable trajectory, concatenated
#    OUTPUTS::
...
#    intra = 0
#    inter = 0
#    for i in tqdm(range(fspace.shape[0]-1)):
#        dists = np.linalg.norm( fspace[i]-fspace[1+1:], axis=1)
#        ss = np.sum( dists[ np.where(mdtrj[i+1:]==mdtrj[i])[0] ] )
#        intra += dd
#        inter += np.sum(dists)-ss
#
#    return intra/inter
