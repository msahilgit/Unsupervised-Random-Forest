...
for i in range(len(randoms)):
    ...
    #model
    inp = Input(shape=(755,))
    en = Dense(200, activation=actv, kernel_initializer=kini)(inp)
    out = Dense(755, activation=actv, kernel_initializer=kini)(en)
    ...
    ae.save(f'saved_tanh_ae4/ae_{i}.h5')
    encoder.save(f'saved_tanh_ae4/encoder_{i}.h5')
    np.save(f'saved_tanh_ae4/loss_{i}.npy', tae.history['loss'])
    np.save(f'saved_tanh_ae4/val_loss_{i}.npy', tae.history['val_loss'])
...
np.save('saved_tanh_ae4/hists.npy', hists)
np.save('saved_tanh_ae4/extents.npy', extents)
