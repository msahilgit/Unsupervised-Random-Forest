...
for i in range(len(randoms)):
    ...
    #model
    inp = Input(shape=(755,))
    e1 = Dense(512, activation=actv, kernel_initializer=kini)(inp)
    e2 = Dense(256, activation=actv, kernel_initializer=kini)(e1)
    en = Dense(200, activation=actv, kernel_initializer=kini)(e2)
    d1 = Dense(256, activation=actv, kernel_initializer=kini)(en)
    d2 = Dense(512, activation=actv, kernel_initializer=kini)(d1)
    out = Dense(755, activation=actv, kernel_initializer=kini)(d2)
    ...
    ae.save(f'saved_tanh_ae1/ae_{i}.h5')
    encoder.save(f'saved_tanh_ae1/encoder_{i}.h5')
    np.save(f'saved_tanh_ae1/loss_{i}.npy', tae.history['loss'])
    np.save(f'saved_tanh_ae1/val_loss_{i}.npy', tae.history['val_loss'])
...
np.save('saved_tanh_ae1/hists.npy', hists)
np.save('saved_tanh_ae1/extents.npy', extents)
