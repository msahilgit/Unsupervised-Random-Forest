...
tic2 = np.load('1_datas/tic2_t4l.npz')
tic2 = [tic2[i] for i in list(tic2)]
dist0 = np.loadtxt('1_datas/distances0.xvg', comments=['@','#'], usecols=[3])
...
for s in hidden_sizes:
    ...
    for d in hidden_depths:
        ...
        model.fit(tic2)
        encoded = model.transform([tic2[0]])[0]
        np.save(f'saved_t4l/encoded_{s}_{d}.npy', encoded)
        
        hh = nh.hist_range(encoded, dist0, mini=0, maxi=0.6)
        h1.append(hh[4])
        e1.append(hh[:4])
    hists.append(h1)
    extents.append(e1)

np.save('saved_t4l/hists.npy', hists)
np.save('saved_t4l/extents.npy', extents)
