...
# hyperparameters
nodes = [features.shape[1], 64, 64, 4]
activ_type = 'tanh'
loss_type = 'sum2'
n_eig = 2
num_epochs = 200
...
# save model
torch.save(model.state_dict(), 'saved_dt5/model_state_dict.pth')  #model.load_state_dict(torch.load('saved_dt5/model_state_dict.pth'))
torch.save(opt.state_dict(), 'saved_dt5/optimizer_state_dict.pth') #opt.load_state_dict(torch.load('saved_dt5/optimizer_state_dict.pth'))
#
np.save(f'saved_dt5/loss_train.npy', model.loss_train)
np.save(f'saved_dt5/loss_valid.npy', model.loss_valid)
np.save(f'saved_dt5/evals_train.npy', np.asarray(torch.cat(model.evals_train)) )

with torch.no_grad():
    dtic2 = model(torch.Tensor(traj_data)).numpy()
np.save(f'saved_dt5/dtic2.npy', dtic2)

hh = nh.hist_range(dtic2, dists, mini=0, maxi=0.6)
np.save('saved_dt5/hists.npy', hh[4])
np.save('saved_dt5/extents.npy', hh[:4])
