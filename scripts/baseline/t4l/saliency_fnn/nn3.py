...
for i in range(len(randoms)):
    ...
    model = tf.keras.Sequential([
        layers.Dense(64, activation='relu', input_shape=(755,)),
    ...
    for v in ['loss', 'val_loss', 'accuracy', 'val_accuracy']:
        np.save(f'saved_nn3/{v}_{i}.npy', history.history[v])
    model.save(f'saved_nn3/nn1_{i}.h5')

    xtf = tf.convert_to_tensor(features, dtype=tf.float32)
    fimp = get_saliency(model, xtf)
    fimp = tf.reduce_mean(fimp, axis=0)
    np.save(f'saved_nn3/fimp_{i}.npy', fimp)
...
np.save('saved_nn3/accu.npy', accu)
np.save('saved_nn3/hists.npy', hists)
np.save('saved_nn3/extents.npy', extents)
