...
for i in range(len(randoms)):
    ...
    for v in ['loss', 'val_loss', 'accuracy', 'val_accuracy']:
        np.save(f'saved_l1_nn1_0.01/{v}_{i}.npy', history.history[v])
    model.save(f'saved_l1_nn1_0.01/nn1_{i}.h5')

    xtf = tf.convert_to_tensor(features, dtype=tf.float32)
    fimp = get_saliency(model, xtf)
    fimp = tf.reduce_mean(fimp, axis=0)
    np.save(f'saved_l1_nn1_0.01/fimp_{i}.npy', fimp)
...
np.save('saved_l1_nn1_0.01/accu.npy', accu)
np.save('saved_l1_nn1_0.01/hists.npy', hists)
np.save('saved_l1_nn1_0.01/extents.npy', extents)
