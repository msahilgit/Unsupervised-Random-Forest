...
for i in range(len(randoms)):
    ...
    model = tf.keras.Sequential([
        layers.Dense(1, activation='sigmoid', input_shape=(755,)),
        ])
    ...
    for v in ['loss', 'val_loss', 'accuracy', 'val_accuracy']:
        np.save(f'saved_nn6/{v}_{i}.npy', history.history[v])
    model.save(f'saved_nn6/nn1_{i}.h5')

    xtf = tf.convert_to_tensor(features, dtype=tf.float32)
    fimp = get_saliency(model, xtf)
    fimp = tf.reduce_mean(fimp, axis=0)
    np.save(f'saved_nn6/fimp_{i}.npy', fimp)
...
np.save('saved_nn6/accu.npy', accu)
np.save('saved_nn6/hists.npy', hists)
np.save('saved_nn6/extents.npy', extents)
