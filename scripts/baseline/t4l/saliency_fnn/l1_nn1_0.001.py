...
for i in range(len(randoms)):
    ...
    model = tf.keras.Sequential([
        layers.Dense(256, activation='relu', input_shape=(755,), kernel_regularizer=l1(reg_term)),
        layers.Dense(128, activation='relu', kernel_regularizer=l1(reg_term)),
        layers.Dense(64, activation='relu', kernel_regularizer=l1(reg_term)),
        layers.Dense(32, activation='relu', kernel_regularizer=l1(reg_term)),
        layers.Dense(16, activation='relu', kernel_regularizer=l1(reg_term)),
        layers.Dense(8, activation='relu', kernel_regularizer=l1(reg_term)),
        layers.Dense(4, activation='relu', kernel_regularizer=l1(reg_term)),
        layers.Dense(2, activation='relu', kernel_regularizer=l1(reg_term)),
        layers.Dense(1, activation='sigmoid', kernel_regularizer=l1(reg_term))
        ])
    ...
    for v in ['loss', 'val_loss', 'accuracy', 'val_accuracy']:
        np.save(f'saved_l1_nn1_0.001/{v}_{i}.npy', history.history[v])
    model.save(f'saved_l1_nn1_0.001/nn1_{i}.h5')

    xtf = tf.convert_to_tensor(features, dtype=tf.float32)
    fimp = get_saliency(model, xtf)
    fimp = tf.reduce_mean(fimp, axis=0)
    np.save(f'saved_l1_nn1_0.001/fimp_{i}.npy', fimp)
...
np.save('saved_l1_nn1_0.001/accu.npy', accu)
np.save('saved_l1_nn1_0.001/hists.npy', hists)
np.save('saved_l1_nn1_0.001/extents.npy', extents)
