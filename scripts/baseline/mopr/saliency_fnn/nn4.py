...
for i in range(len(randoms)):
    ...
    model = tf.keras.Sequential([
        layers.Dense(16, activation='relu', input_shape=(229,)),
    ...
    for v in ['loss', 'val_loss']:
        np.save(f'saved_nn4/{v}_{i}.npy', history.history[v])
    model.save(f'saved_nn4/nn1_{i}.h5')

    xtf = tf.convert_to_tensor(features, dtype=tf.float32)
    fimp = get_saliency(model, xtf)
    fimp = tf.reduce_mean(fimp, axis=0)
    np.save(f'saved_nn4/fimp_{i}.npy', fimp)
...
np.save('saved_nn4/accu.npy', accu)
np.save('saved_nn4/hists.npy', hists)
np.save('saved_nn4/extents.npy', extents)
