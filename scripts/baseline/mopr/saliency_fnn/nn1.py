...
for i in range(len(randoms)):
    ...
    for v in ['loss', 'val_loss']:
        np.save(f'saved_nn1/{v}_{i}.npy', history.history[v])
    model.save(f'saved_nn1/nn1_{i}.h5')

    xtf = tf.convert_to_tensor(features, dtype=tf.float32)
    fimp = get_saliency(model, xtf)
    fimp = tf.reduce_mean(fimp, axis=0)
    np.save(f'saved_nn1/fimp_{i}.npy', fimp)
...
np.save('saved_nn1/accu.npy', accu)
np.save('saved_nn1/hists.npy', hists)
np.save('saved_nn1/extents.npy', extents)
