...
for i in range(len(randoms)):
    ...
    #model
    inp = Input(shape=(229,))
    e1 = Dense(64, activation=actv, kernel_initializer=kini)(inp)
    en = Dense(40, activation=actv, kernel_initializer=kini)(e1)
    d1 = Dense(64, activation=actv, kernel_initializer=kini)(en)
    out = Dense(229, activation=actv, kernel_initializer=kini)(d1)
    ...
    ae.save(f'saved_relu_ae2/ae_{i}.h5')
    encoder.save(f'saved_relu_ae2/encoder_{i}.h5')
    np.save(f'saved_relu_ae2/loss_{i}.npy', tae.history['loss'])
    np.save(f'saved_relu_ae2/val_loss_{i}.npy', tae.history['val_loss'])
...
np.save('saved_relu_ae2/hists.npy', hists)
np.save('saved_relu_ae2/extents.npy', extents)
