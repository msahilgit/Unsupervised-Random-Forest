...
features = np.concatenate((features))[::4]
...
for i in range(len(randoms)):
    ...
    clfs.append(clf_)
    pkl.dump(clf_, open(f'saved_urf_all/clf_permute{i}.pkl','wb'))

np.save('saved_urf_all/accu_permute.npy', accu)


permute_pmt = pmt.calculate_condenced_pmt_multi( clfs, features, n_jobs=94 )
np.save('saved_urf_all/permute_pmt_mean.npy', permute_pmt)

hc_permute = scc.hierarchy.linkage(permute_pmt, method='average')
np.save('saved_urf_all/hc_permute.npy', hc_permute)
...
for h in range(2,11):
    ...
    for i in range(len(randoms)):
        ...
        xtrain, xtest, ytrain, ytest = 0, 0, 0, 0
        pkl.dump(clf_, open(f'saved_urf_all/clf_hlabel{h}_{i}.pkl','wb'))
        fimp = clf_.feature_importances_
        np.save(f'saved_urf_all/fimp_hlabel{h}_{i}.npy', fimp)

np.save('saved_urf_all/accu_hc.npy', accu)
