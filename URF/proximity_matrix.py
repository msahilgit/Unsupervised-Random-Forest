...
def calculate_condenced_pmt_fast(model, data, output_type='horvath'):
    ...
    cite1 - doi 10.1198/106186006X94072
    '''
    nobs = len(data)
...
def calculate_condenced_pmt_fast_multi(model, data, output_type='horvath', n_jobs=1):
    '''
    not good
    '''
    ...
    if isinstance(model, (rfc, rfr) ) and hasattr(model, 'estimator_') :
        ...
        raise ValueError('model(arg1) should be trained RF model(s)')
    ...
    global operate_tree
    def operate_tree(i):
        pmt = forest[i].apply(data)
        return np.equal.outer(pmt, pmt) * 1
        
    with Pool(processes=n_jobs) as pool:
        pmt = list( tqdm( pool.imap(operate_tree, range(ntrees) ), total=ntrees, desc='calculating pmt') )

    pmt = np.sum(pmt, axis=0)
    pmt = pmt/ntrees
...
def calculate_condenced_pmt_multi(model, data, output_type='horvath', n_jobs=1, ftype=np.float64):
    '''
    This function calculates proximity matrix of a given data
    ...
    #INPUTS::
        ...
        ftype       - [type] d[np.float64] the float type of output, control the resulting standing memnory
                            default is float64, but float32 or float16 significantly reduces the memory
                            NOTE: practically usefull for n_jobs=1
...
def calculate_condenced_pmt_multi2(model, data, n_jobs=1,):
    ''' same as above (calculate_condenced_pmt_multi) but parallelization with joblib,
    same in memory usage, but good in time. '''
    # checking data
    if not isinstance(data, np.ndarray) or len(data.shape) != 2 :
        raise ValueError('data(arg2) should 2-dim array similar/same as training data for model')
    nobs = len(data)
    ...
    if not found_aleafs:
        ...
        def operate_tree(tree, data):
            return tree.apply(data)

        aleafs = np.column_stack(( Parallel(n_jobs=n_jobs)(delayed(operate_tree)(tree, data) for tree in forest) ))


    # calculating proximities
    def operate_row(aleafs, row, ntrees):
        return np.sqrt( 1 - np.sum(aleafs[row] == aleafs[row+1:], axis=1)/ntrees )

    pmt = np.concatenate(( Parallel(n_jobs=n_jobs)(delayed(operate_row)(aleafs, row, ntrees) for row in range(nobs-1)) ))
...
def calculate_pmt_many_to_many(leafs, di, dj):
    '''
    this function estimates mean RF based proximities between data clusters (di and dj, for HC),
    returns mean pmt values of horvath type.
    '''
...
def calculate_large_pmt(model, data, output_type='horvath', 
    ...
    # NOTE: TO USE THIS, INSTALL AND REMOVE COMMENT (#) FOR THE LINE (import tables as tb)
...
def calculate_condenced_pmt_(model, data, n_jobs=1):
    '''
    This function calculates proximity matrix of a given data

    NOTE: this function is most economical, but did not allow user-control (like n_jobs, dtype, output_type etc), 
        see other function like calculate_condenced_pmt_multi

    #INPUTS::
        model       - [---] trained RF classifier,regressor or list of it, or leaf nodes
        data        - [arr] input data to calculate proximity - same/subset/full data used to train model
        n_jobs      - [int] d[1] number of cpus for parallel processing
    ...
    if not found_aleafs:
        ...
        with Pool(processes=n_jobs) as pool:
            aleafs = list( pool.imap(operate_tree, range(ntrees)) )
                    
    aleafs = np.column_stack(( aleafs ))

    set_num_threads(n_jobs)
    nobs = len(aleafs)
    if nobs>=10: _ = _get_pmt(leafs=aleafs[:10]) #warm-up for njit compilation

    return _get_pmt(leafs=aleafs)


@njit(parallel=True)
def _get_pmt(leafs):
    nobs = leafs.shape[0]
    ntrees = leafs.shape[1]

    dists = np.empty( np.int64(nobs*(nobs-1)/2) )
    indices = np.zeros((nobs))
    indices[1:] = np.cumsum( np.arange( nobs-1, 0, -1) )
    indices = indices.astype(np.int64)

    for row in prange(nobs-1):
        dists[indices[row]:indices[row+1]] =  np.sqrt( 1 - np.sum(np.equal(leafs[row], leafs[row+1:]), axis=1)/ntrees )
